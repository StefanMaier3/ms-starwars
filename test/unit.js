...
describe("Film", function () {
  ...
  it("should return all the metadata for a film when called", async function () {
    ...
    const vehicleId = "4";
    const speciesId = "1";
    sinon.stub(swapi, "film").withArgs(filmId).resolves(swapiFilmMock);
    sinon.stub(swapi, "people").withArgs(peopleId).resolves(swapiPeopleMock);
    sinon.stub(swapi, "planet").withArgs(planetId).resolves(swapiPlanetMock);
    sinon
      .stub(swapi, "starship")
      .withArgs(starshipId)
      .resolves(swapiStarshipMock);
    sinon.stub(swapi, "vehicle").withArgs(vehicleId).resolves(swapiVehicleMock);
